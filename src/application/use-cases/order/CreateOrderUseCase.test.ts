...
describe('CreateOrderUseCase', () => {
  ...
  const mockCustomerRepo = (): jest.Mocked<ICustomerRepository> => ({
    findById: jest.fn(), findByEmail: jest.fn(), existsByEmail: jest.fn(), findAll: jest.fn(), save: jest.fn(), delete: jest.fn(),
  });
  ...
  it('注文を作成する', async () => {
    ...
    const result = await new CreateOrderUseCase(orderRepo, customerRepo)
      .execute({ customerId: customer.getId().getValue() });
  ...
  context('when 顧客が見つからない', () => {
    it('エラーを投げる', async () => {
      ...
      await expect(new CreateOrderUseCase(mockOrderRepo(), customerRepo).execute({ customerId: 'x' }))
